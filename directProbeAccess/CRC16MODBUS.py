#! /usr/bin/pipenv-shebang

## 
# @file CRC16MODBUS.py
# @brief CRC16MODBUS calculator, might be used for DF1 as well
# @date 2023-07-05
# @auther Manuel Faatz
...
class CRC16:
    def __init__(self):
        ...
        # if you calculate a CRC from the beginning, use these partial functions
        self.calcByteStart = partial(self.calcByte, crc=self.INITIAL_MODBUS)
        self.calcStringStart = partial(self.calcString, crc=self.INITIAL_MODBUS)
        self.calcStringDF1Start = partial(self.calcString, crc=self.INITIAL_DF1)
        self.swapped_calc_stringStart = partial(self.swappedCalcString, crc=self.INITIAL_MODBUS)
        self.calcBytesStart = partial(self.calcBytes, crc=self.INITIAL_MODBUS)
        self.swappedCalcBytesStart = partial(self.swappedCalcBytes, crc=self.INITIAL_MODBUS)

        # if you only want to calculate one or more steps of a CRC, use the full functions below
        # they take the current CRC as an argument

    def calcByte(self, ch: chr, crc: str):
    ...
    def calcString(self, st: str, crc: str):
